...
while True:
    ...
    scanned_type_counts = {}
    scanned_color_counts = {}

    # Compter les types et les couleurs des créatures scannées visibles
    for _id_scanned in scanned_ids:
        if saved_ids:
            for id_saved in saved_ids:
                if _id_scanned in gamestate.visible_creatures:
                    creature = gamestate.visible_creatures[_id_scanned]
                    scanned_type_counts[creature.type] = scanned_type_counts.get(creature.type, 0) + 1
                    scanned_color_counts[creature.color] = scanned_color_counts.get(creature.color, 0) + 1
        else:
            if _id_scanned in gamestate.visible_creatures:
                creature = gamestate.visible_creatures[_id_scanned]
                scanned_type_counts[creature.type] = scanned_type_counts.get(creature.type, 0) + 1
    ...
    print(f"Scanned Type Counts: {scanned_type_counts}", file=sys.stderr, flush=True)
    print(f"Scanned Color Counts: {scanned_color_counts}", file=sys.stderr, flush=True)
    print(f"Non Scanned_Ones = {non_scanned_creatures}", file=sys.stderr, flush=True)

    # Vérifier si tous les types ou couleurs ont atteint le seuil défini
    all_types_scanned = any(count >= TYPE for count in scanned_type_counts.values())
    all_colors_scanned = any(count >= COLORS for count in scanned_color_counts.values())

    print(f"All types scanned: {all_types_scanned}", file=sys.stderr, flush=True)
    print(f"All colors scanned: {all_colors_scanned}", file=sys.stderr, flush=True)

    if all_types_scanned or all_colors_scanned:
        remove_item(scanned_type_counts, scanned_color_counts)
        gamestate.scanned_creatures.clear()
        for drone in gamestate.my_drones:
            if drone.battery >= 5:
